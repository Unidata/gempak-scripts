...
/*
IMPORTANT: We will be checking the filesize of the image after we create it.
We do this to give ourselves a chance to correct blank images if the grids aren't fully in yet.
We only do this here, in this one product script... Which doesn't seem ideal to me, but whatever.

Setting the attempt number to 0, if it reaches 5 we'll give up:
*/
...
# Check filesize and re-run if it's too small:
if ((filesize("$cwd/$ImageFileName") < 3500) && ($Attempt <= 5)) {
	unlink("$cwd/$ImageFileName");
	sleep(3);
	$Attempt++;
	goto RunGempak;
}
